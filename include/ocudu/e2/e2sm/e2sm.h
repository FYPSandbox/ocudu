...
public:
  ...
  /// \brief whether this report service wakes up the indication loop on an event, rather than only periodically.
  /// \return Returns true if event-triggered reporting is supported.
  virtual bool supports_event_trigger() const { return false; }
  /// \brief event signal the indication loop can await for event-triggered reporting.
  /// \return Returns the event flag; only valid to call if supports_event_trigger() is true.
  virtual manual_event_flag& get_report_event_signal()
  {
    static manual_event_flag dummy;
    return dummy;
  }
};
